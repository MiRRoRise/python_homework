...
def start_enigma(rotor_input, reflector_input, rotor_pos, plugboard_input, text_input):
    ...
    try:
        rotors = []
        if len(rotor_pos) != 3 or not all(c in [chr(i) for i in range(ord('А'), ord('Я') + 1)] for c in rotor_pos):
            raise ValueError("Положения роторов должны состоять из 3 заглавных символов кириллицы")
        if not all(c in [chr(i) for i in range(ord('А'), ord('Я') + 1)] or c.isspace() for c in plugboard_input):
            raise ValueError("Конфигурация соединительной панели должна состоять только из заглавных символов кириллицы и пробелов")
        if not all(c in [chr(i) for i in range(ord('А'), ord('Я') + 1)] or c.isspace() for c in text_input):
